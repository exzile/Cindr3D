...
| New **edge-modification tool** (fillet/chamfer family) | Shared core `utils/geometry/edgeCutCore.ts` (parse `featureId\|meshUuid:a:b` IDs, edge→face resolve, exterior-bisector gizmo dir, generic CSG driver `computeEdgeCutGeometry(srcGeo, edges, makeCutter, tag)`). Per-tool module is JUST the cutter: `filletGeometry.ts` (prism−cylinder), `chamferGeometry.ts` (ExtrudeGeometry triangular wedge + `makeBasis(u1,u2,edgeDir)` — same winding-safe pattern as fillet's Box/Cyl). Commit path is shared: `store/cad/slices/featureManagement/{resolveBodySource,applyEdgeCut}.ts` — `commitFillet`/`commitChamfer` in `featureMeshActions.ts` are ~10-line `applyEdgeCut(...)` delegations. R3F is shared too: `viewport/scene/edgeOp/{EdgeOpEdgeHighlight,EdgeOpGizmo,EdgeOpPreview}.tsx`; `Fillet*`/`Chamfer*` scene files are thin store-config wrappers (pass edgeIds + live-value accessors + colour). Store fields per tool: `<tool>EdgeIds`, `add/remove<Tool>Edge`, `<tool>LiveRadius|LiveDistance` + setter in `generalUiActions.ts` (reset in `setActiveDialog`), types in `state/modelingState.ts`. To add a new edge tool: write one cutter fn + one geometry entrypoint, add `commitX` delegation, 3 thin scene wrappers, store fields, register in `Viewport.tsx`, wire the dialog (live-value sync like FilletDialog/ChamferDialog). Pick feedback (hover/selected PULSE opacity + crosshair cursor) is `viewport/scene/pickPulse.ts` (`usePickCursor`, `pulseFactor`, `applyLinePulse`) — shared by the edge-op highlight AND face pickers (`useSimpleFacePicker` + ShellFacePicker/HoleFacePicker). |
