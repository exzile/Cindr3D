...
- `thicken.ts` `computeBodyNormal` + `fillet.ts` `computeChordLengthRadius` (2026-05-30, found in the OCC-19 broad re-scan): both fed an UNCAST `explorer.Current()`/`occDeref` Shape into `BRepAdaptor_Surface_2` — which is **type-strict** (needs a real `TopoDS_Face`, confirmed by the OCC-18 offsetFaces "wrong normal" fix). It threw (swallowed) → the normals array stayed empty → silent wrong output: **thicken always extruded along +Z** (computeBodyNormal returned the `[0,0,1]` fallback, so thickening any non-XY surface went the wrong way) and **chord-length fillet always used the 90° fallback** instead of the true dihedral angle. Fixed with `occ.TopoDS.Face_1(...)` before the adaptor (VIEW — not deleted; thicken also dropped a bogus `rawFace.delete()`). **Reminder: `BRepAdaptor_Surface_2`/`BRepAdaptor_Curve_2` are type-strict — a raw `explorer.Current()` copy is Shape-typed in embind and is NOT accepted; always Face_1/Edge_1-cast first even though Current() returns an owned copy.**
