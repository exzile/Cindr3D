...
- **`apiKey` is intentionally not persisted** (see `aiAssistantStore.ts` `partialize`). The user re-enters it per session. Don't add it to the persisted slice "for convenience" — it would leak in public demo builds.
