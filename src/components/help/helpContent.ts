...
export const TUTORIAL_PROGRESS_KEY = 'cindr3d:tutorial-progress:v1';
...
export const HELP_TOPICS: HelpTopic[] = [
...

  {
    ...
    sections: [
      ...
      {
        heading: 'Dashboard Print Preview card',
        intro: 'The Print Preview dashboard card is a compact live-print situational-awareness panel. It uses the latest sliced toolpath plus live printer state when available.',
        items: [
          'View controls - use the icon buttons for isometric, top, front, side, and fit-to-print recovery views. The card remembers the last view and color mode for that printer/browser session.',
          'Layer focus - the active layer is bright, previous layers are dimmed, and future layers are hidden. Drag the mini layer scrubber to inspect another layer, then click Live to return to firmware-driven progress.',
          'Scrubber shortcuts - while the layer slider is focused, arrow keys step layers, Shift+arrow jumps by ten, Home/End jump to first/last layer, mouse wheel steps layers, and L returns to live mode.',
          'Color modes - switch between Type, Speed, Layer Time, Extrusion, Width, and Object. Modes that use a scale show a compact legend in the card.',
          'Objects - hover a silhouette for name, dimensions, material estimate, status, and the first printability warning. Right-click an object to open the cancel menu where firmware support exists.',
          'Status badges - in-scene labels call out printing, queued, cancelled, or at-risk objects using Duet object data, Klipper status, M486 labels, and printability warnings where available.',
          'Nozzle marker - when live XYZ coordinates are exposed by the firmware, the card shows a small nozzle/crosshair marker and short motion trail above the plate.',
          'Diagnostics - slicer issue markers highlight risk points on the current layer, such as bridge, thin-wall, adhesion, travel-ratio, or layer-time warnings.',
          'Stats strip - the bottom strip shows layer, Z height, progress, active object, elapsed time, ETA, and issue count without opening the full Job tab.',
          'Camera sync - the sync button emits the current preview layer, object, view, and bounds for camera/AR overlays that listen for dashboard preview state.',
        ],
        notes: [
          'If the card has no slice result, it falls back to the build plate and object silhouettes only. Re-slice in Prepare to refresh toolpath, diagnostics, and material estimates.',
          'If the nozzle marker is missing, the connected firmware is not currently exposing live X/Y/Z coordinates through the printer model.',
        ],
      },
      {
      ...
      },
      {
        ...
        items: [
          'Dashboard - heaters, axes, current job, camera summary, quick controls, plus drag-and-resizable panels including the enhanced 3D Print Preview card and an Object Cancellation card.',
    ...
    sections: [
      ...
      },
      {
        ...
        items: [
          ...
          'On the printer dashboard, use the Print Preview card for live layer focus, view recovery, object status badges, nozzle position, diagnostics, and quick per-object cancel without leaving Monitor.',
        ],
      },
    ],
  },
