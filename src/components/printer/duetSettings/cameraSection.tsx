...
export function CameraSection({
  ...
  const cameras =
    prefs.cameras.length > 0 ? prefs.cameras : [legacyCameraFromPrefs(prefs)];
  const activeCamera =
    cameras.find((c) => c.id === prefs.activeCameraId) ?? cameras[0];
  const savedCameraAddress =
    activeCamera.host || cameraAddressFromStreamUrl(activeCamera.url);
  ...
  const [draftRole, setDraftRole] = useState<CameraStreamRole>(
    activeCamera.role,
  );
  ...
  const [draftSourceType, setDraftSourceType] = useState<CameraSourceType>(
    activeCamera.sourceType,
  );
  const [draftStreamUrl, setDraftStreamUrl] = useState(activeCamera.url);
  const [draftMainStreamUrl, setDraftMainStreamUrl] = useState(
    activeCamera.mainStreamUrl,
  );
  const [draftUsbDeviceId, setDraftUsbDeviceId] = useState(
    activeCamera.usbDeviceId,
  );
  const [draftUsbDeviceLabel, setDraftUsbDeviceLabel] = useState(
    activeCamera.usbDeviceLabel,
  );
  const [draftServerUsbDevice, setDraftServerUsbDevice] = useState(
    activeCamera.serverUsbDevice,
  );
  const [draftStreamPreference, setDraftStreamPreference] = useState(
    activeCamera.streamPreference,
  );
  const [draftMainStreamProtocol, setDraftMainStreamProtocol] = useState(
    activeCamera.mainStreamProtocol,
  );
  const [draftRtspTransport, setDraftRtspTransport] = useState(
    activeCamera.rtspTransport,
  );
  const [draftPathPreset, setDraftPathPreset] = useState<CameraPathPreset>(
    activeCamera.pathPreset,
  );
  ...
  const [draftPtzEnabled, setDraftPtzEnabled] = useState(
    activeCamera.ptzEnabled,
  );
  const [draftPtzProvider, setDraftPtzProvider] = useState<CameraPtzProvider>(
    activeCamera.ptzProvider,
  );
  const [draftPtzMoveUrlTemplate, setDraftPtzMoveUrlTemplate] = useState(
    activeCamera.ptzMoveUrlTemplate,
  );
  const [draftPtzPresetUrlTemplate, setDraftPtzPresetUrlTemplate] = useState(
    activeCamera.ptzPresetUrlTemplate,
  );
  const [draftWebRtcEnabled, setDraftWebRtcEnabled] = useState(
    activeCamera.webRtcEnabled,
  );
  const [draftWebRtcUrl, setDraftWebRtcUrl] = useState(activeCamera.webRtcUrl);
  const [draftWebRtcIceServers, setDraftWebRtcIceServers] = useState(
    activeCamera.webRtcIceServers,
  );
  const [testState, setTestState] = useState<CameraTestState>({
    status: "idle",
  });
  const [saved, setSaved] = useState(false);

  const resolvedUrl = useMemo(
    () => normalizeCameraStreamUrl(draftStreamUrl),
    [draftStreamUrl],
  );
  ...
  const addCamera = () => {
    ...
    patchPrefs({
      cameras: [
        ...cameras.map((c) =>
          c.id === draftCameraId ? cameraFromDraft(c.id) : c,
        ),
        nextCamera,
      ],
      activeCameraId: nextCamera.id,
  ...
  const removeCamera = () => {
    ...
    patchPrefs({
      ...
      activeCameraId: nextCamera.id,
      dashboardCameraId:
        prefs.dashboardCameraId === draftCameraId
          ? nextCamera.id
          : prefs.dashboardCameraId,
      ...cameraToLegacyPrefs(nextCamera),
    });
    loadCameraDraft(nextCamera);
  };
  ...
  const loadBrowserUsbDevices = () => {
    if (!navigator.mediaDevices?.enumerateDevices) {
      setTestState({
        status: "error",
        url: "",
        message: "This browser cannot list USB cameras.",
      });
      return;
    }
        ...
        setTestState(
          cams.length
            ...
            : {
                status: "error",
                url: "",
                message: "No USB cameras were found by this browser.",
              },
        );
      })
      .catch(() => {
        setTestState({
          status: "error",
          url: "",
          message: "Browser camera permission is required to list USB cameras.",
        });
      });
  };

  const handleTestCamera = () => {
    const candidateUrls = cameraStreamCandidates(
      draftAddress,
      draftStreamUrl,
      hostname,
      draftPathPreset,
    ...
    const attempts = candidateUrls
      .flatMap((sourceUrl) => {
        const displayUrl = cameraDisplayUrl(
          sourceUrl,
          draftUsername,
          draftPassword,
        );
      ...
      .filter(
        (attempt, index, all) =>
          attempt.testUrl &&
          all.findIndex((item) => item.testUrl === attempt.testUrl) === index,
      );
  ...
  const handleSaveCamera = () => {
    ...
    const nextCameras = cameras.map((c) =>
      c.id === savedCamera.id ? savedCamera : c,
    );
    patchPrefs({
  ...
  return (
    <>
      ...
      <div className="duet-settings__banner duet-settings__banner--info">
        <Camera size={16} /> Configure a network camera, browser USB camera, or
        server USB camera for this printer.
      </div>
