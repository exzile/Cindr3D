/**
 * DirectEditFacePicker — face picking for the Direct Edit dialog (D123).
 *
 * Active when activeDialog === 'direct-edit' && directEditFaceId === null.
 * Hover=blue highlight, click → setDirectEditFace(centroid string).
...
export default function DirectEditFacePicker() {
  ...
  const pickEnabled = activeDialog === 'direct-edit' && directEditFaceId === null;
  const overlayEnabled = activeDialog === 'direct-edit';
  ...
  useFrame(({ scene }) => {
    ...
    if (directEditFaceId && selectedBoundaryRef.current && !selectedMeshRef.current) {
      const mesh = new THREE.Mesh(buildFaceGeometry(selectedBoundaryRef.current), SELECTED_MAT);
      mesh.renderOrder = 100;
      scene.add(mesh);
      selectedMeshRef.current = mesh;
    }
    if (!directEditFaceId && selectedMeshRef.current) {
