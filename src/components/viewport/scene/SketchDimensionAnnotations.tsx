  ...
  const annotations = useMemo<AnnData[]>(() => {
    ...
    const resolveDimensionSegment = (id: string) => {
      ...
      if (
        ...
        if (!Number.isInteger(edgeIndex) || edgeIndex < 0 || edgeIndex > 3)
          return null;
        ...
        return {
          start: {
            ...
            x: corners[(edgeIndex + 1) % corners.length]
              .clone()
              .sub(origin)
              .dot(t1),
            y: corners[(edgeIndex + 1) % corners.length]
              .clone()
              .sub(origin)
              .dot(t2),
          },
    ...
    const buildTwoLineAnnotation = (
      ...
      const firstHorizontal =
        Math.abs(first.end.x - first.start.x) >=
        Math.abs(first.end.y - first.start.y);
      const secondHorizontal =
        Math.abs(second.end.x - second.start.x) >=
        Math.abs(second.end.y - second.start.y);
      ...
      if (firstHorizontal) {
        ...
        const firstNearX =
          Math.abs(first.start.x - x) <= Math.abs(first.end.x - x)
            ? first.start.x
            : first.end.x;
        const secondNearX =
          Math.abs(second.start.x - x) <= Math.abs(second.end.x - x)
            ? second.start.x
            : second.end.x;
        return {
          pairs: [
      ...
      const firstNearY =
        Math.abs(first.start.y - y) <= Math.abs(first.end.y - y)
          ? first.start.y
          : first.end.y;
      const secondNearY =
        Math.abs(second.start.y - y) <= Math.abs(second.end.y - y)
          ? second.start.y
          : second.end.y;
      return {
        pairs: [
    ...
    const buildTwoLineAngleAnnotation = (
      firstId: string,
      secondId: string,
      position: Vec2,
    ) => {
      ...
      for (const c of candidates) {
        const mid =
          ((c.annotationArc.startAngle + c.annotationArc.endAngle) / 2 +
            Math.PI * 4) %
          (Math.PI * 2);
    ...
    const computeLinearAnnotationAtPosition = (
      start: Vec2,
      end: Vec2,
      position: Vec2,
      ...
      const offset =
        resolvedOrientation === "horizontal"
          ? position.y - base
          : position.x - base;
      return DimensionEngine.computeLinearDimension(
        start,
        end,
        offset,
        resolvedOrientation,
    ...
    const computeAlignedAnnotationAtPosition = (
      start: Vec2,
      end: Vec2,
      position: Vec2,
    ) => {
    ...
    for (const dim of activeSketch.dimensions) {
      try {
        ...
        if (
          ...
          if (
            (dim.type === "linear" || dim.type === "aligned") &&
            dim.entityIds.length >= 2
          ) {
            ...
            const isDegen = (s: { start: Vec2; end: Vec2 } | null) =>
              s != null &&
              Math.hypot(s.end.x - s.start.x, s.end.y - s.start.y) < 1e-8;
            if (isDegen(seg0) && isDegen(seg1)) {
              const ann =
                dim.type === "linear"
                  ? computeLinearAnnotationAtPosition(
                      seg0!.start,
                      seg1!.start,
                      dim.position,
                      dim.orientation ?? "auto",
                    )
                  : computeAlignedAnnotationAtPosition(
                      seg0!.start,
                      seg1!.start,
                      dim.position,
                    );
              ...
              const segs = makeSegments(
                [
                  ann.extensionLine1,
                  ann.extensionLine2,
                  ...withArrowheads(ann.dimensionLine),
                ],
                origin,
                t1,
                t2,
            ...
            if (
              ...
              const { foot } = pointToLineDistance(
                pointSeg.start,
                lineSeg.start,
                lineSeg.end,
              );
              const ann = computeAlignedAnnotationAtPosition(
                pointSeg.start,
                foot,
                dim.position,
              );
            ...
            const twoLineAnn = buildTwoLineAnnotation(
              dim.entityIds[0],
              dim.entityIds[1],
              dim.position,
            );
            if (twoLineAnn) {
              const pairs = [
                ...
                ...withArrowheads(
                  twoLineAnn.pairs[twoLineAnn.pairs.length - 1],
                ),
              ];
          ...
          if (dim.type === "angular" && dim.entityIds.length >= 2) {
            const angleAnn = buildTwoLineAngleAnnotation(
              dim.entityIds[0],
              dim.entityIds[1],
              dim.position,
            );
          ...
          if (pts.length < 2) {
            pts.push(dim.position, {
              x: dim.position.x + dim.value,
              y: dim.position.y,
            });
          }
          ...
          if (dim.type === "linear") {
            ...
            const ann = computeLinearAnnotationAtPosition(
              pts[0],
              pts[1],
              dim.position,
              dim.orientation ?? "auto",
            ...
            const ray1End = allPts[1] ?? {
              x: dim.position.x + OFFSET,
              y: dim.position.y,
            };
            const ray2End = allPts[2] ?? {
              x: dim.position.x,
              y: dim.position.y + OFFSET,
            };
            const ann = DimensionEngine.computeAngleDimension(
              vertex,
              ray1End,
              ray2End,
              OFFSET,
            ...
            for (let i = 0; i < SEGS; i++) {
              const a0 = startAngle + (i / SEGS) * (endAngle - startAngle);
              const a1 =
                startAngle + ((i + 1) / SEGS) * (endAngle - startAngle);
              arcPairs.push([
                { x: cx + r * Math.cos(a0), y: cy + r * Math.sin(a0) },
                { x: cx + r * Math.cos(a1), y: cy + r * Math.sin(a1) },
              ]);
          ...
          const cy = center2d.y;
          const r =
            circEnt.radius ?? dim.value / (dim.type === "diameter" ? 2 : 1);

          if (dim.type === "diameter") {
            const start = { x: cx - r, y: cy };
            const end = { x: cx + r, y: cy };
            const segs = makeSegments(
              withArrowheads([start, end]),
              origin,
              t1,
              t2,
            );
            ...
            const segs = makeSegments(
              withArrowheads([{ x: cx, y: cy }, edgePt]),
              origin,
              t1,
              t2,
            );
          ...
          arcPairs.push(
            [
              {
                x: acx + ar * Math.cos(aStart),
                y: acy + ar * Math.sin(aStart),
              },
              {
                x: acx + annR * Math.cos(aStart),
                y: acy + annR * Math.sin(aStart),
              },
            ],
            [
              { x: acx + ar * Math.cos(aEnd), y: acy + ar * Math.sin(aEnd) },
              {
                x: acx + annR * Math.cos(aEnd),
                y: acy + annR * Math.sin(aEnd),
              },
            ],
          );
          const segs = makeSegments(arcPairs, origin, t1, t2);
          ...
          const ann = DimensionEngine.computeEllipseDimension(
            ec.x,
            ec.y,
            axisEnd,
            dim.type,
          ...
          const ann = DimensionEngine.computeConcentricGapDimension(
            cg1.x,
            cg1.y,
            r1,
            r2,
