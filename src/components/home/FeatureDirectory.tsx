...
export function FeatureDirectory() {
  ...
  useEffect(() => {
    ...
    if (!section || hasSpawned) return undefined;
  ...
  return (
    <section
      ref={sectionRef}
      className={`home-band home-band--features${hasSpawned ? ' fd-animate-in' : ''}`}
      id="features"
            ...
            {featureGroups.map((group, index) => {
              ...
              return (
                <div
                  ...
                  style={{ '--fd-color': color, '--fd-delay': `${index * 55}ms` } as React.CSSProperties}
                >
            ...
            {visibleGroups.map((group, index) => {
              ...
              return (
                <div
                  className="fd-row"
                  key={`${group.page}-${group.section}`}
                  style={{ '--fd-color': color, '--fd-delay': `${index * 70}ms` } as React.CSSProperties}
                >
