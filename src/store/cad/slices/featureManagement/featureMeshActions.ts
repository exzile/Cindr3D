...
export function createFeatureMeshActions({
  ...
  return {
    ...
    commitFillet: (radius, segments, featureId?, filletParams?) => {
      ...
      applyEdgeCut(
        { get, set },
        {
          ...
          compute: (srcGeo, edges) =>
            computeFilletGeometry(srcGeo, edges, radius, segments, false, fp),
          pastVerb: "Filleted",
          sizeLabel: `r=${radius}`,
          featureId,
        },
        ...
        { get, set },
        {
          ...
          compute: (srcGeo, edges) =>
            computeChamferGeometry(
              ...
              distance2,
              false,
              chamferParams as Record<string, unknown> | undefined,
            ),
          pastVerb: "Chamfered",
    ...
    replayEdgeCutFeature: (featureId: string) => {
      ...
      let srcLabel: "cache" | "parent" | "bodyCache" | "live" | "unknown" =
        "unknown";
      ...
      if (cached) {
        ...
        const parentId =
          (params.parentFeatureId as string | undefined) ??
          feature.parentFeatureId;
        const parent = parentId
          ? features.find((f) => f.id === parentId)
          : null;
        if (parent?.mesh instanceof THREE.Mesh) {
          ...
          if (cached2) {
            srcGeo = cached2.clone().toNonIndexed();
            srcLabel = "bodyCache";
          }
        }
        if (!srcGeo && edgeIds.length > 0) {
          const rest = edgeIds[0].includes("|")
            ? edgeIds[0].split("|")[1]
            : edgeIds[0];
      ...
      if (!srcGeo) {
        get().setStatusMessage(
          `${feature.type}: srcGeo unavailable — re-apply`,
        );
        return;
      }
      ...
      try {
        if (feature.type === "fillet") {
          ...
          const fp: FilletCommitParams = {
            ...
            offsetOne: params.offsetOne as number | undefined,
            offsetTwo: params.offsetTwo as number | undefined,
            isFlipped: params.isFlipped as boolean | undefined,
            propagate: params.propagate as boolean | undefined,
            isG2: params.isG2 as boolean | undefined,
            tangencyWeight: params.tangencyWeight as number | undefined,
            isRollingBallCorner: params.isRollingBallCorner as
              | boolean
              | undefined,
          };
          const parsedEdges = parseFilletEdgeIds(edgeIds);
          if (parsedEdges)
            newGeo = computeFilletGeometry(
              srcGeo,
              parsedEdges.edges,
              radius,
              0,
          ...
          const [d1, d2] = resolveChamferDistances({
            mode: ((params.mode as string) ??
              "equal-dist") as import("../../../../utils/geometry/chamferGeometry").ChamferMode,
          ...
          features: state.features.map((f) =>
            f.id === featureId
              ? {
                  ...
                  healthMessage: "CSG× — adjust radius/edges",
                }
              : f,
          ),
        }));
      ...
      const totalCount: number =
        (newGeo.userData.totalEdgeCount as number | undefined) ??
        edgeIds.length;
      const sizeLabel =
        feature.type === "fillet"
          ...
          if (f.id === featureId) {
            return {
              ...
              params: {
                ...
                targetId,
                toolId,
                booleanParentIds: [targetId, toolId],
                recomputeOnParentChange: true,
              },
            };
          }
