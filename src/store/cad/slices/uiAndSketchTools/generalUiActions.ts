...
export function createGeneralUiActions({
  set,
  get,
  ...
  return {
    ...
    removeFilletEdge: (id) =>
      set((state) => ({
        filletEdgeIds: state.filletEdgeIds.filter((e) => e !== id),
      })),
    ...
    removeChamferEdge: (id) =>
      set((state) => ({
        chamferEdgeIds: state.chamferEdgeIds.filter((e) => e !== id),
      })),
    ...
    setSelectionFilter: (f) =>
      set((state) => ({ selectionFilter: { ...state.selectionFilter, ...f } })),
    ...
    removeParameter: (id) => {
      set((state) => ({
        parameters: resolveParameters(
          state.parameters.filter((p) => p.id !== id),
        ),
      }));
    },
    ...
    createComponentPattern: (sourceId, type, params) => {
      ...
      for (let i = 1; i < n; i++) {
        ...
        if (type === "linear") {
          ...
          offsetMatrix = new THREE.Matrix4().makeRotationAxis(
            axisVec(params.circularAxis),
            angle,
          );
        }

        // Create new child component for this copy
        const newCompId = componentStore.addComponent(
          parentId,
          `${source.name} (${i + 1})`,
        );
        ...
        for (const bodyId of source.bodyIds) {
          ...
          const newBodyId = componentStore.addBody(
            newCompId,
            `${srcBody.name} (${i + 1})`,
          );
      ...
      get().setStatusMessage(
        `Component pattern: ${n - 1} cop${n - 1 === 1 ? "y" : "ies"} created`,
      );
    },
  };
}
