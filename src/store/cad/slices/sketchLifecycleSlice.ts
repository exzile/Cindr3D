...
export function createSketchLifecycleSlice({ set, get }: CADSliceContext) {
  ...
  sliceSketch: (sketchId) => {
    ...
    // Find the selected body's OCC representation
    ...
    // brepBodyId lives on the leaf Mesh, not on a Group node — check instanceof first
    const brepBodyId =
    ...
    // Bug 12: getOccSync() returns OcctInstance { oc: OcctRaw } — pass the raw module
    ...
    // If this sketch is the active sketch, update it; otherwise patch the stored sketch.
    // Bug 4: use a functional updater so concurrent dispatches don't tear.
    get().pushUndo();
