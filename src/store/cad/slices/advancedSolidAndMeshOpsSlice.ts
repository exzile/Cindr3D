...
export function createAdvancedSolidAndMeshOpsSlice({ set, get }: CADSliceContext) {
  ...
  commitShell: (featureId, opts) => {
    ...
    // Prefer stored mesh; fall back to live scene mesh for CSG-pipeline bodies
    // (standard solid extrudes never store feature.mesh — they render via ExtrudedBodies).
    ...
    const toVec = (a: [number, number, number]) => new THREE.Vector3(a[0], a[1], a[2]);
    const toSpec = (d: { normal: [number, number, number]; centroid: [number, number, number]; boundary: [number, number, number][]; thickness?: number }) => ({
      normal: toVec(d.normal),
      centroid: toVec(d.centroid),
      boundary: d.boundary.map(toVec),
      ...(d.thickness != null ? { thickness: d.thickness } : {}),
    });
    get().pushUndo();
    let result: THREE.Mesh;
    try {
      result = GeometryEngine.shellSolid(srcMesh, {
        insideThickness: tin,
        outsideThickness: tout,
        shellType,
        removeFaces: removeFaces.map(toSpec),
      ...
      f.id === featureId
        ? {
            ...
            params: {
              ...
              outsideThickness: tout,
              shellType,
              removeFaceCount: removeFaces.length,
              featureKind: 'shell',
            },
          }
        : f,
    ...
    set({ features: nextFeatures });
    const openTxt = removeFaces.length ? `, ${removeFaces.length} open face(s)` : '';
    get().setStatusMessage(`Shell applied (in ${tin}mm / out ${tout}mm${openTxt})`);
  },
  ...
  commitSplitBody: ({ bodyFeatureId, toolType, toolId }) => {
    ...
    // Resolve mesh — stored on feature OR registered in liveBodyMeshes
    let srcMesh = srcFeature?.mesh as THREE.Mesh | undefined;
    if (!srcMesh?.isMesh) {
      for (const [, m] of liveBodyMeshes) {
    ...
    // Map construction plane name → normal
