...
const MODEL_FOOTPRINTS: Readonly<Record<string, [number, number]>> = {
...
function fmtStepValue(param: string | undefined, value: number): string {
  switch (param) {
    case 'temperature':
...
// ── Preset-override display helpers ──────────────────────────────────────────
...
function fmtPresetValue(key: string, value: unknown): string {
...
// ── Tuning-plane hover info ───────────────────────────────────────────────────

interface PlaneHoverInfo {
  z: number;
  value: number;
...
// ── Tuning-plane R3F sub-components ──────────────────────────────────────────

/**
 * Semi-transparent horizontal plane at a given Z height that marks where a
 * tuning-tower post-processor injects a change (temperature, PA, fan, etc.).
 * Brightens on hover and fires `onHoverChange` so the parent can show a tooltip.
 */
...
/**
 * Renders a TuningPlane at every Z height where a tuning-tower processor will
 * inject a G-code change command.
 */
function TuningPlaneMarkers({
  processors,
  cx,
  cy,
  planeW,
