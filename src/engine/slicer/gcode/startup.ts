...
export function appendHeaderPlaceholders(
  ...
  // OrcaSlicer / PrusaSlicer wrap their metadata in HEADER_BLOCK_START
  // / HEADER_BLOCK_END so external tools (Octoprint plugins, Klipper
  // preview, gcode post-processors) can find it without scanning the
  // body. We mirror that wrap and emit the same field names so the
  // tools that already parse Orca's gcode work with ours unchanged.
  gcode.push('; HEADER_BLOCK_START');
  gcode.push('; Generated by Cindr3D Slicer');
  gcode.push('; PRINT_TIME_PLACEHOLDER');
  gcode.push('; FILAMENT_USED_PLACEHOLDER');
  // total layer number / max_z_height come from `prepareMeshGeometry`,
  // which runs AFTER the header is appended. We emit placeholders and
  // let `finalizeGCodeStats` patch them once the values are known.
  gcode.push('; total layer number: TOTAL_LAYERS_PLACEHOLDER');
  gcode.push('; max_z_height: MAX_Z_HEIGHT_PLACEHOLDER');
  gcode.push(`; filament_diameter: ${printer.filamentDiameter}`);
  gcode.push(`; filament_density: ${material.density}`);
...
export function appendStartGCode({
  ...
  if (printer.startGCodeMustBeFirst && startGCode.trim()) {
    // `startGCodeMustBeFirst` printers (Klipper START_PRINT etc.)
    // require the macro line to be at gcode[0]. The macro itself
    // handles its own classification, so we don't pre-pend a marker;
    // it's the runtime expansion that emits the actual moves.
  ...
  if (!printer.startGCodeMustBeFirst && normalizedStartGCode) {
    // ;TYPE:Custom — see comment on the matching emit above. The
    // dedupe pass may produce an empty string when every line in the
    // template overlapped with what we already emitted; we only flag
    // this block when there's real content left.
