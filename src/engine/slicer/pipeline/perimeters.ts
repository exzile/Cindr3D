...
// ARACHNE-9.4A.4: slicer worker awaits `loadClipper2Module()` before the
// first slice fires (see `src/workers/SlicerWorker.ts`), so the sync
// boolean adapters always return a real result. Throw if not — that
// indicates the worker contract was violated, not a transient miss.
function requireMP(result: PCMultiPolygon | null, op: string): PCMultiPolygon {
  if (result === null) {
    throw new Error(`perimeters.${op}: Clipper2 WASM not loaded — slicer worker must await loadClipper2Module() before slicing`);
  }
  return result;
}
